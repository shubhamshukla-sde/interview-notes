...
* **New Model:** If `Virtual Thread 1` waits for a DB response, the JVM moves it to the **Heap** (Unmounted). `Carrier Thread 1` is now free to pick up `Virtual Thread 2`. We get high throughput with just ONE OS thread.
